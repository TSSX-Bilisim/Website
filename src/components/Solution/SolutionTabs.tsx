...
const SolutionTabs = ({ onNavigate, className }: SolutionTabsProps) => {
  const { t } = useTranslation('solutions');
  // Reuse existing section ids: overview, services, use-cases, screenshots, success
  const tabs = useMemo(() => ([
    { key: 'overview', label: t('tabs.overview'), target: 'overview' },
    { key: 'services', label: t('tabs.services'), target: 'services' },
    { key: 'screens', label: t('tabs.screenshots'), target: 'screenshots' },
    { key: 'success', label: t('tabs.success'), target: 'success-stories' }
  ]), [t]);
  ...
  useEffect(() => {
    ...
    const calcActive = () => {
      ...
      const anchor = window.scrollY + headerHeight + navHeight + 4;
  ...
  return (
    <nav
      ref={navRef}
      id="solution-tabs"
      aria-label={t('solution_tabs_label') + ' tabs'}
      style={{ top: offsetTop }}
      className={'w-full bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 sticky z-40 transition-shadow ' +
      ...
      {device === 'mobile' && (
        <div className="px-4 py-4">
          <button
            ...
            aria-expanded={open}
            aria-controls="solution-mobile-tablist"
          ...
          <div className={'overflow-hidden transition-[max-height,opacity] duration-300 ' + (open ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0')} id="solution-mobile-tablist">
